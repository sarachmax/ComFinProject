...
def parseStockPrices(from_date, to_date, symbol):
    ...
    # yahoo = Share(symbol)
    hist_prices = yf.download(symbol, from_date, to_date)
    # print(len(hist_prices))
    # print(len(hist_prices.iloc[0]))
    np_hist_prices = np.empty(shape=[len(hist_prices), len(hist_prices.iloc[0])])
    i=0
    # for record in hist_prices:
    for index, record in hist_prices.iterrows():
        np_hist_prices[i,0]= float(record['Adj Close'])
        np_hist_prices[i,1]=float(record['Close'])
        np_hist_prices[i,2]=float(record['High'])
        np_hist_prices[i,3]=float(record['Low'])
        np_hist_prices[i,4]=float(record['Open'])
        np_hist_prices[i,5]=float(record['Volume'])
        i+=1
...
if __name__ == '__main__':
    ...
    hist_prices = parseStockPrices('2016-08-17', '2016-08-30', '^VIX')  # yyyy-mm-dd
